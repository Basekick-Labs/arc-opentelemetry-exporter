...
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time, value, labels->'\''cpu'\'' as cpu, labels->'\''state'\'' as state FROM system_cpu_time ORDER BY time DESC LIMIT 20","format":"json"}'

# Query memory utilization
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time, value, labels->'\''state'\'' as state FROM system_memory_usage ORDER BY time DESC LIMIT 10","format":"json"}'

# Query disk I/O
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time, value, labels->'\''device'\'' as device, labels->'\''direction'\'' as direction FROM system_disk_io ORDER BY time DESC LIMIT 10","format":"json"}'

# Query network traffic
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time, value, labels->'\''device'\'' as device, labels->'\''direction'\'' as direction FROM system_network_io ORDER BY time DESC LIMIT 10","format":"json"}'
```
...
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT labels->'\''cpu'\'' as cpu, labels->'\''state'\'' as state, avg(value) as avg_value FROM system_cpu_time WHERE time > now() - INTERVAL '\''1 minute'\'' GROUP BY labels->'\''cpu'\'', labels->'\''state'\'' ORDER BY cpu, state","format":"json"}'

# Memory usage summary
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT labels->'\''state'\'' as state, avg(value) / 1024 / 1024 / 1024 as avg_gb FROM system_memory_usage WHERE time > now() - INTERVAL '\''5 minutes'\'' GROUP BY labels->'\''state'\''","format":"json"}'

# Disk usage by filesystem
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT labels->'\''device'\'' as device, labels->'\''mountpoint'\'' as mountpoint, avg(value) / 1024 / 1024 / 1024 as avg_gb FROM system_filesystem_usage WHERE time > now() - INTERVAL '\''5 minutes'\'' AND labels->'\''state'\'' = '\''used'\'' GROUP BY labels->'\''device'\'', labels->'\''mountpoint'\''","format":"json"}'

# Network throughput
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time_bucket(INTERVAL '\''1 minute'\'', time) as minute, labels->'\''device'\'' as device, labels->'\''direction'\'' as direction, sum(value) / 1024 / 1024 as mb FROM system_network_io WHERE time > now() - INTERVAL '\''10 minutes'\'' GROUP BY minute, device, direction ORDER BY minute DESC, device, direction","format":"json"}'
```
...
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time, labels->'\''cpu'\'' as cpu, value FROM system_cpu_time WHERE time > now() - INTERVAL '\''1 hour'\'' AND labels->'\''state'\'' = '\''user'\'' AND value > 50 ORDER BY time DESC","format":"json"}'

# Memory pressure events
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time_bucket(INTERVAL '\''5 minutes'\'', time) as period, avg(value) / 1024 / 1024 / 1024 as avg_used_gb, max(value) / 1024 / 1024 / 1024 as max_used_gb FROM system_memory_usage WHERE time > now() - INTERVAL '\''1 hour'\'' AND labels->'\''state'\'' = '\''used'\'' GROUP BY period HAVING avg(value) / 1024 / 1024 / 1024 > 8 ORDER BY period DESC","format":"json"}'

# High load average periods
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -H "x-arc-database: metrics" \
  -d '{"sql":"SELECT time, value as load_avg_1m FROM system_cpu_load_average_1m WHERE time > now() - INTERVAL '\''1 hour'\'' AND value > 4.0 ORDER BY time DESC","format":"json"}'
```
