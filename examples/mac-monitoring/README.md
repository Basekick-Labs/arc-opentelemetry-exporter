...
# Query CPU metrics (note: direct column access, no JSON parsing!)
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT time, value, cpu, state FROM metrics.system_cpu_time ORDER BY time DESC LIMIT 20","format":"json"}'

# Query memory utilization
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT time, value, state FROM metrics.system_memory_usage ORDER BY time DESC LIMIT 10","format":"json"}'

# Query disk I/O
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT time, value, device, direction FROM metrics.system_disk_io ORDER BY time DESC LIMIT 10","format":"json"}'

# Query network traffic
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT time, value, device, direction FROM metrics.system_network_io ORDER BY time DESC LIMIT 10","format":"json"}'
```
...
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT cpu, state, avg(value) as avg_value FROM metrics.system_cpu_time WHERE time > now() - INTERVAL '\''1 minute'\'' GROUP BY cpu, state ORDER BY cpu, state","format":"json"}'

# Memory usage summary
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT state, avg(value) / 1024 / 1024 / 1024 as avg_gb FROM metrics.system_memory_usage WHERE time > now() - INTERVAL '\''5 minutes'\'' GROUP BY state","format":"json"}'

# Disk usage by filesystem
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT device, mountpoint, avg(value) / 1024 / 1024 / 1024 as avg_gb FROM metrics.system_filesystem_usage WHERE time > now() - INTERVAL '\''5 minutes'\'' AND state = '\''used'\'' GROUP BY device, mountpoint","format":"json"}'

# Network throughput
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT time_bucket(INTERVAL '\''1 minute'\'', time) as minute, device, direction, sum(value) / 1024 / 1024 as mb FROM metrics.system_network_io WHERE time > now() - INTERVAL '\''10 minutes'\'' GROUP BY minute, device, direction ORDER BY minute DESC, device, direction","format":"json"}'
```
...
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT time, cpu, value FROM metrics.system_cpu_time WHERE time > now() - INTERVAL '\''1 hour'\'' AND state = '\''user'\'' AND value > 50 ORDER BY time DESC","format":"json"}'

# Memory pressure events
curl -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"sql":"SELECT time_bucket(INTERVAL '\''5 minutes'\'', time) as period, avg(value) / 1024 / 1024 / 1024 as avg_used_gb, max(value) / 1024 / 1024 / 1024 as max_used_gb FROM metrics.system_memory_usage WHERE time > now() - INTERVAL '\''1 hour'\'' AND state = '\''used'\'' GROUP BY period HAVING avg(value) / 1024 / 1024 / 1024 > 8 ORDER BY period DESC","format":"json"}'
...
Traditional OTel exporters store labels as a single JSONB column:
```sql
-- Old approach (JSONB labels)
SELECT time, value, labels->>'cpu', labels->>'state'
FROM metrics.system_cpu_time
WHERE labels->>'state' = 'user'
```

**Issues:**
- Requires JSON parsing on every query
- Can't use regular indexes efficiently
- Multiple rows per timestamp (one per label combination)
- Poor query performance on large datasets

### Our Solution: Dynamic Column Extraction

The Arc exporter **automatically detects all label keys** and creates explicit columns for them:

```sql
-- New approach (explicit columns)
SELECT time, value, cpu, state
FROM metrics.system_cpu_time
WHERE state = 'user'
```
...
- ✅ Can use PostgreSQL indexes on columns
- ✅ Better query planner optimization
- ✅ Much faster WHERE clause filtering
- ✅ Cleaner, more readable SQL
- ✅ Fully dynamic - any label becomes a column automatically

### How the Exporter Works

1. **Batch Collection**: Groups metrics by name (each metric = one table)
...
// Each label key becomes a column
for labelKey := range labelKeys {
    ...
    for i, labels := range batch.labels {
        if val, ok := labels[labelKey]; ok {
            ...
            columnValues[i] = nil  // Arc handles nulls
        }
    }
...
Your `system_cpu_utilization` table looks like this:

| time | value | cpu | state |
|------|-------|-----|-------|
| 2025-11-13 19:17:18 | 0.192 | cpu0 | user |
| 2025-11-13 19:17:18 | 0.514 | cpu0 | system |
| 2025-11-13 19:17:18 | 0.293 | cpu0 | idle |
